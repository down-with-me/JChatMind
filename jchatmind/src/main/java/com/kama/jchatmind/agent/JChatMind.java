...
public class JChatMind {
    ...
    // 发送状态更新
    private void sendState(SseMessage.Type type, String statusText) {
        SseMessage sseMessage = SseMessage.builder()
                .type(type)
                .payload(SseMessage.Payload.builder()
    ...
    private boolean think() {
        ...
        sendState(SseMessage.Type.AI_THINKING, "AI is thinking...");

        String thinkPrompt = "You are an intelligent 'Decision Module'.\\n" +
                "Please decide the next action based on the current conversation context.\\n" +
                "                \\n" +
                "[Extra Information]\\n" +
                "- List of knowledge bases you invoke and their descriptions: %s\\n" +
                "- If context is missing, prioritize searching from the knowledge base.\\n";

        thinkPrompt = thinkPrompt.formatted(this.availableKbs);

        // Include thinkPrompt via .system() or .user() constructs
        ...
        // If tool calls is not empty, enter execution phase
    ...
    private void execute() {
        ...
        sendState(SseMessage.Type.AI_EXECUTING, "AI is executing tools...");
        ...
        String collect = toolResponseMessage.getResponses()
                .stream()
                .map(resp -> "Tool " + resp.name() + " execution result: " + resp.responseData())
                .collect(Collectors.joining("\n"));

        log.info("Tool execution result: {}", collect);
    ...
    public void run() {
        ...
        try {
            ...
            agentState = AgentState.FINISHED;
            sendState(SseMessage.Type.AI_DONE, "Reply finished");
        } catch (Exception e) {
